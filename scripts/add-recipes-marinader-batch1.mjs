...
${dish} starter med den rigtige flaske: ${r.wineInRecipe.amount}. ${r.wineInRecipe.style} giver syre og aroma, der mørner og bærer krydderierne ind i råvaren. Læs ${guides} og [madlavning med vin](/guides/sadan-bruger-du-vin-til-sauce-og-simren). Opskriften dækker ca. ${r.servings} portioner kød/fisk/grønt.

${specific}

## Smagsbalance og justering

For syrlig marinade: mere olie eller en snert honning. For mild: mere vin, eddike eller citrus. Salt tidligt mørner, men tør altid overfladen før stegning/grill. Til glasset: [${r.wineToDrink.label}](/guides/${r.wineToDrink.guideSlug}). Se også [rødvinsmarinade til oksekød](/opskrifter/rodvinsmarinade-til-oksekod) for grundprincipper.`;
}
...
${tagStr} — vinmarinade er en af de nemmeste måder at få dybde uden lang simring. ${r.wineInRecipe.note} Følg tiderne første gang; tynde udskæringer mørner hurtigere end tykke stege.

Resten af flasken hører til glasset: [${r.wineToDrink.label}](/guides/${r.wineToDrink.guideSlug}). Stil: ${r.wineInRecipe.style.toLowerCase()}. God appetit — og husk at tørre råvaren før varme.`;
}
