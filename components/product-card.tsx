...
export function ProductCard({ product, placement = "home-search" }: { product: ProductHit; placement?: string }) {
  ...
  const referencePrice =
    product.referencePrice != null && product.discountPercent != null
    ...
    <article className="flex flex-col overflow-hidden rounded-2xl border border-stone-200/90 bg-white shadow-sm transition hover:shadow-md">
      <div className="relative">
        {product.discountPercent != null && product.discountPercent >= 10 ? (
          <span className="absolute left-3 top-3 z-10 rounded-full bg-rose-900 px-2 py-0.5 text-[11px] font-semibold text-white">
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        {price && (
          <div className="flex flex-wrap items-baseline gap-2">
            <p className="text-lg font-semibold text-stone-800">{price}</p>
            {referencePrice ? (
              <p className="text-sm text-stone-500 line-through">{referencePrice}</p>
            ) : null}
          </div>
        )}
        <a
