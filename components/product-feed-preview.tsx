...
export async function ProductFeedPreview({
  ...
  const { products, source } = await runSearch(query, maxPrice ?? null);

  if (!products.length) {
    return (
      <section className="rounded-2xl border border-stone-200 bg-stone-50 p-6 text-stone-600">
        <p className="font-medium text-stone-800">{title || "Produkter fra feeds"}</p>
        <p className="mt-2 text-sm">Ingen matchende vine lige nu ({source}). Prøv en anden søgning eller kom tilbage senere.</p>
      </section>
    );
  }
