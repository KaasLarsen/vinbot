...
import {
  expandWineGlassQuery,
  productIsWineGlass,
  WINE_GLASS_MERCHANTS,
  wineGlassSearchHayMatchesTerm,
} from "./wine-glass";
...
const glassFeeds = FEEDS.filter((f) => WINE_GLASS_MERCHANTS.includes(f.merchant));
if (!glassFeeds.length) {
  throw new Error("Ingen vinglas-feeds i FEEDS-konfigurationen");
}
...
export async function runWineGlassSearch(qRaw: string, budgetMaxParam: number | null): Promise<SearchResult> {
  const priceFilter = parsePriceFilter(qRaw, budgetMaxParam);
  const { min: priceMin, max: priceMax } = priceFilter;
  const terms = expandWineGlassQuery(qRaw);
  ...
  const lists = await Promise.all(
    glassFeeds.map(async (feed) => {
      try {
        let products = await getCachedFeedProducts(feed);
        products = products.filter(productIsWineGlass);
        ...
        const matches =
          query.length === 0
            ? products
            : products.filter((p) => terms.some((t) => wineGlassSearchHayMatchesTerm(p._search || "", t)));
        ...
        feeds_failed++;
        console.error(
          `[wine-glass-search] feed FAILED for ${feed.merchant}:`,
  ...
  return {
    source: feeds_failed === glassFeeds.length && feeds_ok === 0 ? "error" : "fallback",
    products: [],
    meta,
  };
}
